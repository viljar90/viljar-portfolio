...
function App() {
  ...
  // This effect determines if we should render the full portfolio or a standalone project page.
  useEffect(() => {
...
const PortfolioApp = ({ darkMode, toggleDarkMode }) => {
    ...
    const landing = useLandingChapter(currentChapter, navigatedManually, isReady);
    const design = useDesignChapter(currentChapter, isReady);
    const work = useWorkChapter(currentChapter, isReady);
    const me = useMeChapter(currentChapter);
  
    const [mainChapterAnimClass, setMainChapterAnimClass] = useState('');
    const [designChapterAnimClass, setDesignChapterAnimClass] = useState('');
    const [workChapterAnimClass, setWorkChapterAnimClass] = useState('');
    const [meChapterAnimClass, setMeChapterAnimClass] = useState('');
  
    // Effect 1: INITIALIZATION & STATE SYNC FROM URL (for back/forward buttons)
    useEffect(() => {
      const syncStateFromUrl = () => {
        ...
        const targetChapter = ['main', 'design', 'work', 'me'].includes(chapter) ? chapter : 'main';

        setCurrentChapter(targetChapter);

        if (isInitialLoad.current) {
          setSnappedChapter(targetChapter);
          let targetRef;
          if (targetChapter === 'main') targetRef = mainChapterRef;
          else if (targetChapter === 'design') targetRef = designChapterRef;
          else if (targetChapter === 'work') targetRef = workChapterRef;
          else if (targetChapter === 'me') targetRef = meChapterRef;
        ...
        // Always sync sub-chapter state
        if (targetChapter === 'main') {
          const mainNavItem = MAIN_NAV_ITEMS.find(item => item.slug === subChapter);
          if (mainNavItem) landing.setActiveMainStep(mainNavItem.name);
        } else if (targetChapter === 'design') {
           if (subChapter === 'what') {
                if (design.designView !== DESIGN_VIEWS.WHAT_DESIGN) design.setDesignView(DESIGN_VIEWS.WHAT_DESIGN);
                const targetStageItem = WHAT_DESIGN_NAV_ITEMS.find(item => item.title && item.title.toLowerCase().replace(/\s+/g, '-') === section);
                if (targetStageItem) design.navigateToStage(targetStageItem.name, false);
            } else if (subChapter === 'why') {
                if (design.designView !== DESIGN_VIEWS.WHY_DESIGN) design.setDesignView(DESIGN_VIEWS.WHY_DESIGN);
                if (section === 'score') design.setGameStatus('end');
                else if (section === 'bonus') {
                    design.setGameStatus('bonus');
                    if (rest[0] === 'case' && rest[1]) design.navigateToGameCase(rest[1]);
                }
                else if (section === 'case' && rest[0]) design.navigateToGameCase(rest[0]);
                else if (section === 'start') design.returnToIntro();
            }
        } else if (targetChapter === 'work') {
          if (subChapter === 'overview') {
            if (work.workView !== 'Overview') work.setWorkView('Overview');
            const projectIndex = PROJECTS.findIndex(p => p.id === section);
            if (projectIndex !== -1) work.setCurrentProjectIndex(projectIndex);
          } else {
            if (work.workView !== 'Quiz') work.setWorkView('Quiz');
            if (section === 'start') work.setWorkStepIndex(0);
            else if (section === 'results') work.setWorkStepIndex(QUIZZES.length + 1);
            else {
              const quizIndex = QUIZZES.findIndex(q => q.slug === section);
              if (quizIndex !== -1) work.setWorkStepIndex(quizIndex + 1);
            }
          }
        }
      };
    ...
    }, []); // This runs only once to set up the system.

    // Effect 2: STATE-TO-URL SYNC (The Central URL Manager)
    useEffect(() => {
      if (!isReady || isInitialLoad.current) return;
  
      let newHash = '';
      if (currentChapter === 'main') {
        const navItem = MAIN_NAV_ITEMS.find(item => item.name === landing.activeMainStep);
        newHash = `#main/${navItem?.slug || 'insults'}`;
      } else if (currentChapter === 'design') {
          if (design.designView === DESIGN_VIEWS.WHAT_DESIGN) {
              const navItem = WHAT_DESIGN_NAV_ITEMS.find(item => item.name === design.activeDesignStageKey);
              if (navItem?.title) {
                  const sectionSlug = navItem.title.toLowerCase().replace(/\s+/g, '-');
                  newHash = `#design/what/${sectionSlug}`;
              }
          } else {
              let whySlug = 'start';
              if (design.whyDesignStep === 'game') {
                  const currentCase = design.WHY_DESIGN_GAME_CONTENT[design.gameCaseIndex];
                  if (design.gameStatus === 'end') whySlug = 'score';
                  else if (design.gameStatus === 'bonus') {
                      whySlug = 'bonus';
                      if (design.gameCaseIndex >= 3 && currentCase) whySlug += `/case/${currentCase.slug}`;
                  }
                  else if (design.gameStatus === 'playing' && currentCase) whySlug = `case/${currentCase.slug}`;
              }
              newHash = `#design/why/${whySlug}`;
          }
      } else if (currentChapter === 'work') {
          if (work.workView === 'Overview') {
              const projectSlug = PROJECTS[work.currentProjectIndex]?.id;
              newHash = `#work/overview${projectSlug ? `/${projectSlug}` : ''}`;
          } else {
              let quizSlug = 'start';
              if (work.workStepIndex > 0 && work.workStepIndex <= QUIZZES.length) {
                  quizSlug = QUIZZES[work.workStepIndex - 1]?.slug || `question-${work.workStepIndex}`;
              } else if (work.workStepIndex > QUIZZES.length) {
                  quizSlug = 'results';
              }
              newHash = `#work/quiz/${quizSlug}`;
          }
      } else if (currentChapter === 'me') {
          newHash = '#me';
      }
    ...
    }, [isReady, currentChapter, landing.activeMainStep, design.designView, design.activeDesignStageKey, design.whyDesignStep, design.gameStatus, design.gameCaseIndex, work.workView, work.workStepIndex, work.currentProjectIndex]);

    // Effect 3: ANIMATION CONTROLLER
    useEffect(() => {
      if (!isReady) return; // <-- This is the key to preventing animation on load.
  
      const duration = ANIMATION_DURATION_CHAPTER;
      setMainChapterAnimClass(currentChapter === 'main' ? `animate-[slideUpIn_${duration}_ease-out_forwards]` : `animate-[slideDownOut_${duration}_ease-in_forwards] pointer-events-none`);
      setDesignChapterAnimClass(currentChapter === 'design' ? `animate-[slideUpIn_${duration}_ease-out_forwards]` : `animate-[slideDownOut_${duration}_ease-in_forwards] pointer-events-none`);
      setWorkChapterAnimClass(currentChapter === 'work' ? `animate-[slideUpIn_${duration}_ease-out_forwards]` : `animate-[slideDownOut_${duration}_ease-in_forwards] pointer-events-none`);
      setMeChapterAnimClass(currentChapter === 'me' ? `animate-[slideUpIn_${duration}_ease-out_forwards]` : `animate-[slideDownOut_${duration}_ease-in_forwards] pointer-events-none`);
    }, [isReady, currentChapter]);
  
    // Effect 4: INTERSECTION OBSERVER
    useEffect(() => {
      if (!isReady) return;
      ...
      const refs = [mainChapterRef, designChapterRef, workChapterRef, meChapterRef];
      refs.forEach(ref => ref.current && observer.observe(ref.current));
  
      return () => refs.forEach(ref => ref.current && observer.unobserve(ref.current));
    }, [isReady, currentChapter]);

    // [NO CHANGE to the rest of the file from here on]
    // The following code is your existing logic and does not need to be changed.
