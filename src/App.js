...
const PortfolioApp = ({ darkMode, toggleDarkMode }) => {
    ...
    useEffect(() => {
      const syncStateFromUrl = () => {
        ...
        } else if (targetChapter !== currentChapter) {
           setCurrentChapter(targetChapter);
        }

        const params = new URLSearchParams(queryString);
        const state = params.get('state');
        if (targetChapter === 'work' && subChapter === 'quiz' && state) {
          try {
            const restoredAnswers = JSON.parse(decodeURIComponent(state));
            work.setQuizAnswers(restoredAnswers);
          } catch (e) {
            console.error("Failed to parse quiz state from URL:", e);
          }
        }
        ...
        newHash = `#main/${navItem?.slug || 'insults'}`;
          ...
          if (design.designView === DESIGN_VIEWS.WHAT_DESIGN) {
              ...
              if (design.whyDesignStep === 'game') {
                  const currentCase = WHY_DESIGN_GAME_CONTENT[design.gameCaseIndex];
